...
\subsubsection*{5.9.3.7 BMC 跨 run 資料修正與回溯敏感度}
...
\item BMC E12 因 6 個 development files 未達 30-row 門檻而未評估 final test；E13 為 parser-invalidated。E14C 雖在修正後回溯資料上顯著優於 baseline，但 test bytes 已開啟，只能視為候選資格證據；E15 仍為 NOT\_\allowbreak{}EVALUATED。
...
\item 機箱 E11A 至 E11F 已完成分層研究；E11D 支持角色語意的預測資訊，E11F 只支持同一 AAU campaign 的 commissioning-assisted unseen-byte transfer。BMC E14C 候選仍須由尚未執行的 E15 新檔案確認。後續任何跨機箱、跨伺服器、NTC 硬體或氣流因果主張都必須另立 protocol。
\end{itemize}
