...
\item 目前室內受控或估測狀態的溫度建模與應用討論只涵蓋 20–30 °C；外部天氣邊界可超出此區間，但不因此擴張室內適用性主張，其他環境或製程亦不得直接外推。
\item 模型為簡化動態模型，不追求 CFD 等級高精度流場。
\item 濕度保留於模型中，但作為次核心變數處理。
\item 人體舒適以目標值與容許範圍表示；低估測誤差本身不等於需要同等精度的實體舒適控制。
...
\subsection*{2.9 動態精準環境應用與 Kalman Filter 方向}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.83\textwidth]{assets/fig_3_1_research_logic.png}
\par\smallskip
{\small 圖 3-1 研究整體邏輯架構。此圖將研究缺口、RQ1--RQ4、方法核心、E1--E9 證據層與 claim boundary 串成同一條論證鏈；其中 RQ1--RQ3 為主要研究線，RQ4 為次要服務線，E8 維持 future intervention protocol。}
\end{figure}
...
\hline
window\_\allowbreak{}summer\_\allowbreak{}sunny\_\allowbreak{}noon & 37.0 & 71.0 & 36000.0 & 247.3004 \\
\hline
window\_\allowbreak{}winter\_\allowbreak{}rainy\_\allowbreak{}night & 11.0 & 78.0 & 15.2 & 73.0820 \\
\hline
window\_\allowbreak{}spring\_\allowbreak{}cloudy\_\allowbreak{}morning & 21.5 & 70.0 & 5005.0 & 96.7992 \\
\hline
\end{tabular}
\end{table}

\subsection*{5.7 【實驗 E6】Pure RNN 與 Hybrid Residual Neural Network 結果}
...
\par\smallskip
{\small 圖 5-5 IDW、base model、pure RNN 與 LOO hybrid residual correction 的平均 field MAE 比較。圖中使用 log-scale y-axis；pure RNN 與 hybrid 均採 8-fold LOO。}
\end{figure}
...
上述 bootstrap 是針對既有七個日期 block 的描述性不確定性分析，不會增加新的獨立資料，也不能把逐快照改善比例解讀為控制介入成功率。其結論仍限定在 bedroom\_\allowbreak{}01、單一 pillow hold-out 位置與觀察到的七天期間。
...
為避免實驗完成後才改變欄位或指標，本研究已將 E8 protocol 實作成預註冊 execution kit：e8\_\allowbreak{}intervention\_\allowbreak{}trial\_\allowbreak{}schema.json 固定 study、trial、target、before/after observations、predicted ranking、executed action、settling interval、control condition 與 protocol deviations；e8\_\allowbreak{}intervention\_\allowbreak{}trials\_\allowbreak{}template.json 提供 bedroom\_\allowbreak{}01 pillow point 的空白收集格式；analyze\_\allowbreak{}e8\_\allowbreak{}intervention\_\allowbreak{}trials.py 則重新計算 comfort penalty、actual improvement、prediction error、direction accuracy，並僅在同一 block 具有可比較 action arms 時輸出 top-1 regret 與 Spearman rank correlation。


目前執行套件已可重現地產生 e8\_\allowbreak{}intervention\_\allowbreak{}summary.json，但 E8 目前完成真實介入試驗數為 0，證據狀態仍是 `NOT\_\allowbreak{}EVALUATED`，所有效益估計維持 null。這表示資料契約與分析路徑已準備完成，不表示推薦具有實測效果；單元測試使用的 synthetic fixtures 只驗證公式與錯誤處理，不能進入論文證據。
...
\subsubsection*{5.9.3 Oh et al. (2024) 方法移植比較}
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|}
\hline
\textbf{Horizon} & \textbf{Persistence MAE
(dining/room)} & \textbf{Direct LR MAE} & \textbf{Raw physics MAE} & \textbf{本研究 readout MAE} & \textbf{Oh2024-inspired MAE} \\
\hline
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.195\textwidth}|>{\raggedright\arraybackslash}p{0.195\textwidth}|>{\raggedright\arraybackslash}p{0.195\textwidth}|>{\raggedright\arraybackslash}p{0.195\textwidth}|}
\hline
\textbf{次日方法} & \textbf{Dining MAE} & \textbf{Room MAE} & \textbf{相對 persistence} \\
\hline
...
\subsubsection*{5.9.3.2 Vanilla RNN 同資料公平比較}
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.260\textwidth}|>{\raggedright\arraybackslash}p{0.260\textwidth}|>{\raggedright\arraybackslash}p{0.260\textwidth}|}
\hline
\textbf{方法} & \textbf{最低 MAE 案例數} & \textbf{結果解讀} \\
\hline
...
\subsubsection*{GRU 與 LSTM 簡易同資料比較}
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|}
\hline
\textbf{模型} & \textbf{參數量} & \textbf{最低 MAE} & \textbf{勝過 vanilla RNN} & \textbf{中位相對 MAE 改善} \\
\hline
...
\subsubsection*{5.9.3.3 Kalman Filter 受控同資料比較}
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|}
\hline
\textbf{目標族群} & \textbf{案例數} & \textbf{未濾波最低} & \textbf{MA(3) 最低} & \textbf{Linear Kalman 最低} & \textbf{判讀} \\
\hline
...
這項結果的 evidence class 是 CONTROLLED\_\allowbreak{}INJECTED\_\allowbreak{}NOISE。原始 SML2010 量測只作 task reference，並非 latent physical ground truth；受控雜訊也不是 DHT11、DHT22、SHT31 或其他實體感測器的 noise characterization。因此，本節不能支持真實感測器去噪、未來值預測、完整 3D 場、控制效益或跨場域主張。


\subsubsection*{5.9.3.4 機箱 BMC 公開資料轉移比較}
...
完整資料 commit 含 124 個 source CSV，展開為 317 個 file-device cases；其中 312 個因 20–30 °C 內合格 pair 少於 30 而標為 insufficient\_\allowbreak{}in\_\allowbreak{}scope\_\allowbreak{}samples，只有 5 個案例可評估。五案皆為 11 秒 median cadence，結果如下。
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|>{\raggedright\arraybackslash}p{0.130\textwidth}|}
\hline
\textbf{Case} & \textbf{Eligible pairs} & \textbf{Persistence MAE} & \textbf{Linear MAE} & \textbf{Thermal-balance MAE} & \textbf{最低 MAE} \\
\hline
...
\subsubsection*{5.9.3.5 AAU 伺服器機房空間轉移比較}
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|}
\hline
\textbf{基線} & \textbf{MAE（°C）} & \textbf{RMSE（°C）} & \textbf{P95 絕對誤差（°C）} & \textbf{感測器勝出} \\
\hline
...
\subsubsection*{5.9.3.6 E11C 局部鄰域獨立確認}
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.195\textwidth}|>{\raggedright\arraybackslash}p{0.195\textwidth}|>{\raggedright\arraybackslash}p{0.195\textwidth}|>{\raggedright\arraybackslash}p{0.195\textwidth}|}
\hline
\textbf{方法} & \textbf{MAE（°C）} & \textbf{RMSE（°C）} & \textbf{P95 絕對誤差（°C）} \\
\hline
...
\subsubsection*{5.9.3.7 BMC 跨 run 資料修正、回溯敏感度與同伺服器確認}

E12 至 E14C 形成一條必須依序解讀的資料品質與模型證據鏈。E12 在固定 12/5/14 檔 train/selection/test split 上要求每檔至少 30 筆有效 BMC rows；2026-09-07 的結構化重現確認有 6 個 development files 未達門檻，因此未選模、未開啟 final-test files，狀態為 NOT\_\allowbreak{}EVALUATED。E13 將開發門檻改為 10 rows 後雖完成運算，但該結果使用尚未修正的 parser／unit pipeline，現標記為 PARSER\_\allowbreak{}INVALIDATED，不作模型證據。


E14A 以逐 section header 與獨立 oracle 稽核 31 檔，保留 4,038 筆 source-correct BMC rows，但三個 selection files 的 raw hwmon 單位使溫度合理性閘門失敗。E14B 再以固定中位數規則辨識單位 regime，對這三檔套用 10\textasciicircum{}-3 溫度與 10\textasciicircum{}-6 功率縮放，其餘 28 檔維持 scale 1.0；4,038 rows 全數保留，校正後溫度範圍為 29.0–77.5 °C，八個資料品質閘門全通過。E14A/E14B 只支持資料管線正確性，不是模型準確度確認。
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|}
\hline
\textbf{E14C retrospective test} & \textbf{MAE（°C）} & \textbf{RMSE（°C）} & \textbf{P95（°C）} & \textbf{逐 run 勝出} \\
\hline
...
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|>{\raggedright\arraybackslash}p{0.156\textwidth}|}
\hline
\textbf{E15 同伺服器 temporal/workload confirmation} & \textbf{MAE（°C）} & \textbf{RMSE（°C）} & \textbf{P95（°C）} & \textbf{逐 run 勝出} \\
\hline
...
\subsubsection*{5.9.4 CU-BEMS：C1/C2/C3 任務族群拆解}
...
\subsubsection*{5.9.5 綜合判讀與 Claim Boundary}
...
\begin{itemize}
\item 目前已加入小型真實臥室快照、date-block bootstrap 與逐日剔除敏感度分析，但資料仍只有七個日期 block、單一 pillow hold-out，且缺乏長期連續部署資料與 dense spatial ground truth；七個剔除 folds 高度重疊，不能視為獨立重複實驗。
...
\item 目前室內受控或估測狀態的研究溫度範圍為 20–30 °C；外部天氣邊界不擴張此主張，即使候選應用需要動態環境控制，也不得把室內結果外推到此區間以外。
\item 48 組窗戶矩陣中，依 target-zone 室內溫度稽核只有 34 組位於 20–30 °C；其餘 14 組僅作範圍外壓力測試，不得列入目前應用適用性證據。
\item 人體舒適採目標帶與容許範圍，現有低 MAE 不能直接證明一般人居環境需要極窄溫濕度控制。
...
\item Oh et al. (2024) 的 BEMS data 為 confidential，本文只能執行 ridge-linear method transfer，不能重現其 TRNSYS/RC、CNN--LSTM 或 published next-day performance；SML2010 的 24 小時 transfer 亦未顯示 residual 優於 persistence。
\item 次日改善 follow-up 的 fixed 與 adaptive 方法都未通過預註冊判準；validation/test 非平穩漂移與缺少 target-day 邊界/操作 forecast 使現有 SML2010 結果不足以支持 next-day advantage。
\item 固定 vanilla RNN 在同資料的 12 個 SML2010 S2 案例中沒有取得最低 MAE；此結果不能代表所有 recurrent architecture，但也不能省略或改寫為模型改善。
\item 固定 pure Elman RNN 在八情境完整 3-D 場的 24 個 fold×因子比較亦未取得最低 MAE；sensor-token recurrence 不是物理時間序列，結果只限此單房間 controlled synthetic 設定。
\item Kalman 目前只完成 SML2010 固定種子 injected-noise current-time filtering；linear Kalman 與 MA(3) 各在 6/12 案例最低，不能外推為真實 sensing node、forecast、3D field 或 online parameter adaptation 優勢。
\item 機箱 E11A 只完成公開 BMC next-observation outlet-air task；317 個 file-device cases 中 312 個未達 20–30 °C 內最低樣本門檻，5 個可評估案例全部由 persistence 最佳，不能外推到 3-D 熱場、元件 hotspot、PID 或任意設備櫃。
\item BMC E12 因 6 個 development files 未達 30-row 門檻而未評估 final test；E13 為 parser-invalidated。E14C 仍只構成回溯候選資格證據；E15 雖通過十項閘門、12/14 runs 勝出，但只支持同伺服器 temporal/workload transfer，且兩個退步 runs 必須保留。
\item 機箱 E11B/E11C 只完成 AAU 固定 byte-range 的 leave-one-sensor-out 比較；E11C local IDW 雖降低 aggregate error，卻只在 21/42 感測器勝出，不能外推為 CFD、因果控制、完整期間或 topology-aware 模型已驗證。
...
\item 以全程位於 20–30 °C 的小型封閉植物生長環境作為候選情境，先驗證日夜或生長階段 setpoint 的溫濕度、光照追蹤與空間均勻性；在加入 PPFD/PAR、CO2、基質水分、氣流與生物 endpoint 前，不宣稱植物培養成效。
\item 以獨立 validation reference 執行實體 sensing-node filtering，估計 real measurement noise、missingness 與 covariance drift；只有在 nonlinear transition/observation model 明確後才擴展 EKF/UKF 或 online parameter adaptation。
\item GRU 與 LSTM 已完成第一個單一 seed、同資料、近似參數量的 SML2010 簡易比較；兩者最低 MAE 皆為 0/12，GRU 僅 2/12 勝 vanilla 且中位相對改善 -12.880146\%，LSTM 為 0/12 與 -11.368865\%，沒有候選通過門檻。若改 history、容量、seed 或完整 3-D 任務，必須另立 protocol。
\item 將 PID 納入未來閉環控制 baseline；在執行前固定 plant、動態 setpoint、disturbance、sampling、actuator limit 與安全 cutoff，並比較 tracking MAE、settling time、overshoot、control effort 與 constraint violations。PID 不屬於 3-D 場估測器，目前亦為 NOT\_\allowbreak{}EVALUATED。
\item 機箱 E11A 至 E11F 已完成分層研究；E11D 支持角色語意的預測資訊，E11F 只支持同一 AAU campaign 的 commissioning-assisted unseen-byte transfer。BMC E15 已完成同伺服器 temporal/workload confirmation；後續個人電腦機箱實測、跨機箱、跨伺服器、NTC 硬體或氣流因果主張都必須另立 protocol。
\end{itemize}
...
\item [26] Ju-Hong Oh, Stefano Sfarra, Eui-Jong Kim, Hybrid modeling based on integrating simulation and operational data to improve indoor air temperature predictions, a controlled variable in digital twin models, Energy and Buildings, vol. 324, 114898, 2024. DOI: 10.1016/j.enbuild.2024.114898
\item [27] Jeffrey L. Elman, Finding Structure in Time, Cognitive Science, vol. 14, no. 2, pp. 179-211, 1990. DOI: 10.1207/s15516709cog1402\_\allowbreak{}1
\item [28] R. E. Kalman, A New Approach to Linear Filtering and Prediction Problems, Journal of Basic Engineering, vol. 82, no. 1, pp. 35-45, 1960. DOI: 10.1115/1.3662552
\item [29] Camilo Chiang, Daniel Bånkestad, Günter Hoch, Reaching Natural Growth: The Significance of Light and Temperature Fluctuations in Plant Performance in Indoor Growth Facilities, Plants, vol. 9, no. 10, 1312, 2020. DOI: 10.3390/plants9101312
\item [30] Young Ho Kim et al., Preventing Overgrowth of Cucumber and Tomato Seedlings Using Difference between Day and Night Temperature in a Plant Factory with Artificial Lighting, Plants, vol. 12, no. 17, 3164, 2023. DOI: 10.3390/plants12173164
\item [31] Simon van Mourik, Peter J. M. van Beveren, Irineo L. López-Cruz, Eldert J. van Henten, Improving climate monitoring in greenhouse cultivation via model based filtering, Biosystems Engineering, vol. 181, pp. 40-51, 2019. DOI: 10.1016/j.biosystemseng.2019.03.001
\item [32] S. L. Speetjens, J. D. Stigter, G. van Straten, Towards an adaptive model for greenhouse control, Computers and Electronics in Agriculture, vol. 67, no. 1-2, pp. 1-8, 2009. DOI: 10.1016/j.compag.2009.01.012
\item [33] Yewan Wang, Yiru Zhang, David Nörtershäuser, Stéphane Le Masson, Jean-Marc Menaud, Model and data driven transient thermal system modelings for contained data centers, Energy and Buildings, vol. 258, 111790, 2022. DOI: 10.1016/j.enbuild.2021.111790
\item [34] Da Zhang, Haojun Xia, Xiaotong Wang, Yanchang Feng, Haohao Liu, Bibo Tu, Thermal Elasticity-Aware Host Resource Provision for Carbon Efficiency on Virtualized Servers, IEEE Transactions on Computers, vol. 74, no. 11, pp. 3682-3695, 2025. DOI: 10.1109/TC.2025.3603698
\item [35] arealuser, bmcdata: A dataset collected from server's Baseboard Management Controller (BMC), GitHub dataset, commit 24904fa9a9bac49a3f6f3198bb04e1be5e2707ea, 2026. Available: https://github.com/arealuser/bmcdata
\item [36] Simon Pommerencke Melgaard, Thomas Juul, Jesper Ellerbaek Nielsen, Rasmus Lund Jensen, Data from the AAU Server Room, version 4, Zenodo, 2026. DOI: 10.5281/zenodo.19398358
\item [37] Onur Muhammed Sarikaya et al., Energy efficiency enhancement in two European data centers through CFD modeling, Scientific Reports, vol. 15, 24952, 2025. DOI: 10.1038/s41598-025-11048-0
\item [38] L. Li, T. Losser, C. Yorke, R. Piltner, Fast inverse distance weighting-based spatiotemporal interpolation, International Journal of Environmental Research and Public Health, vol. 11, pp. 9101-9141, 2014. DOI: 10.3390/ijerph110909101
\item [39] F. Gao, Fast k-nearest-neighbors calculation for interpolation of radar reflectivity field, Journal of Atmospheric and Oceanic Technology, vol. 26, pp. 1410-1414, 2009. DOI: 10.1175/2009JTECHA1234.1
\item [40] E. Oktavia, Widyawan, I. W. Mustika, Inverse distance weighting and kriging spatial interpolation for data center thermal monitoring, ICITISEE, pp. 69-74, 2016. DOI: 10.1109/ICITISEE.2016.7803050
\item [41] X. Tong et al., A time-varying state-space model for real-time temperature predictions in rack-based cooling data centers, Applied Thermal Engineering, vol. 230, 120737, 2023. DOI: 10.1016/j.applthermaleng.2023.120737
\end{itemize}
...
{\footnotesize
\renewcommand{\arraystretch}{1.25}
\begin{longtable}{|>{\raggedright\arraybackslash}p{0.33\textwidth}|>{\raggedright\arraybackslash}p{0.53\textwidth}|}
\hline
\textbf{名詞} & \textbf{本文用法} \\
\hline
\endfirsthead
...
\section*{E11G tail-safe 自適應開發與空間覆蓋限制}
\addcontentsline{toc}{section}{E11G tail-safe 自適應開發與空間覆蓋限制}
...
在 42 個感測器、12 日與 63,084 筆感測器分鐘評估上，out-of-fold MAE 由 1.1168°C 降至 0.8945°C，RMSE 由 1.7250°C 降至 1.5415°C，P95 由 3.4900°C 降至 3.1013°C；日區塊 MAE 改善 bootstrap 95\% CI 為 [0.1847, 0.2620]°C。然而嚴格感測器勝率僅 21/42，另有 20 個因安全回退而持平、1 個微幅惡化，未達預註冊 26/42。因此決策為 no\_\allowbreak{}candidate\_\allowbreak{}forwarded，E11F 未存取；本結果僅屬適應性開發證據。


\section*{E11H commissioning 開發與 E11F 凍結確認}
\addcontentsline{toc}{section}{E11H commissioning 開發與 E11F 凍結確認}

E11H 以最早兩日目標點真值做 robust residual calibration，第三日選模，後續九日凍結測試。相較 local-IDW，MAE/RMSE/P95 由 1.0958/1.7435/3.5061°C 降至 0.4039/0.6830/1.2900°C；39/42 感測器改善，日區塊 95\% CI 為 [0.4854, 0.9271]°C。多個 Huber slope 位於 0.5 邊界，affine 參數不可解讀為物理係數。


E11F 使用預留 unseen byte ranges 與完全凍結的 42 組模型，不重新校正。MAE/RMSE/P95 由 1.1399/1.7850/3.5735°C 降至 0.3966/0.6723/1.2756°C；39/42 感測器改善，13 日 bootstrap 95\% CI 為 [0.5851, 0.9274]°C。因此 h\_\allowbreak{}enc\_\allowbreak{}05\_\allowbreak{}supported\_\allowbreak{}within\_\allowbreak{}campaign。11 個日期與 E11G 重疊、8 個與 E11H 重疊，故不是跨日期、跨機箱、氣流因果或 NTC 硬體驗證。
