...
\item 機箱 E11A 至 E11C 已完成且三個假說均不支持。若後續研究 sensor-role、rack topology、airflow direction 或非等向性距離，必須以新的資料切分與 OpenSpec 預註冊；不得用 E11C confirmation metrics 回頭選規則，超過 30 °C 的元件熱點仍不在目前適用範圍。
\end{itemize}
