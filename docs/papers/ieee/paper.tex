...
Digital twins provide computational representations of physical systems and are increasingly used in manufacturing, infrastructure, and smart buildings \cite{tao2019digitaltwin,fuller2020digitaltwin}. Building-oriented reviews show strong attention to BIM/BMS connectivity and system architecture, but sparse-sensor spatial estimation in ordinary rooms remains less explored \cite{cespedes2024bdtreview}. For control-oriented indoor modeling, reduced-order and grey-box approaches remain attractive because they are interpretable and tractable compared with CFD \cite{bacher2011heatdynamics,hietaharju2018dynamicmodel,gokhale2022pinnthermal}. Zonal and hybrid indoor models preserve dominant spatial non-uniformity at lower cost \cite{teshome2004zonalreview,megri2022doma,huljak2025hybrid}. Simulation-informed hybrid forecasting has also used forecast-day physical outputs as a baseline and learned the historical simulation--measurement gap for next-day indoor-temperature prediction \cite{oh2024hybridmodeling}; a vanilla recurrent comparator follows the Elman formulation \cite{elman1990structure}. Data-assimilation studies show that finite observations can reconstruct indoor temperature and humidity fields and that sensor placement matters \cite{qian2025dafield,chen2007zonalnetwork}. Controlled-growth studies further motivate dynamic rather than strictly constant temperature, humidity, and light programs \cite{chiang2020naturalgrowth}, while greenhouse Kalman studies report both successful online adaptation and filtering that deteriorated monitoring when model accuracy was insufficient \cite{speetjens2009adaptive,vanmourik2019filtering}. The present work combines these strands by coupling sparse sensing with appliance-aware influence bases and residual calibration. IDW \cite{shepard1968two} is used as a geometry-free interpolation baseline. The service interface is secondary to the modeling contribution, but the same estimator is exposed through local APIs, a web demo, and an MCP-compatible tool layer \cite{mcp2025}.
...
The window matrix contains 48 cases from four times of day, three weather states, and four seasons. It evaluates the window as an environmental variable rather than a binary switch, and the direct-input mode accepts outdoor temperature, outdoor humidity, sunlight, and opening ratio without discretizing the outside condition. Under the current indoor $20$--$30^\circ$C domain, 34 target-zone results are in range and 14 are retained only as out-of-domain stress cases. Representative window-zone illuminance values are 73.08 lux for winter-rain-night, 96.80 for spring-cloud-morning, and 247.30 for summer-sun-noon.

For external comparison, SML2010 \cite{romeu2014sml2010} and CU-BEMS \cite{chitalia2020cubems} support only task-aligned time-series or zone-level tests because neither supplies matched room geometry and dense 3-D truth. The proposed model is strongest on event-delta and longer-horizon temperature tasks, but persistence remains strong for short-window illuminance and high-inertia CU-BEMS zones; no CU-BEMS task beats persistence. Table~\ref{tab:benchmark} retains representative favorable and adverse cases. An Oh et al.-inspired fixed ridge residual transfer \cite{oh2024hybridmodeling} is useful at short horizons but does not reproduce the confidential CNN--LSTM or its next-day advantage. A preregistered next-day follow-up also worsened MAE to 1.629/1.625 versus persistence at 1.517/1.500, with intervals crossing zero. The RNN same-data comparison is complete: the fixed model used the same four-record histories, splits, targets, and test rows in all 12 cases; linear regression was lowest in 7, persistence in 5, and RNN in 0. Thus the recurrent comparison provides no superiority claim.
...
Dense full-field truth is synthetic; robustness checks do not create measured dense truth. E7 covers one room, seven dates, and one held-out point, while public data validate only aligned subtasks. The fixed RNN does not rule out other recurrent designs. Application claims are limited to indoor states within $20$--$30^\circ$C; dynamic plant growth remains a candidate because the model lacks PPFD/PAR, CO$_2$, substrate, airflow, and biological endpoints. Action rankings remain counterfactual pending interventions.

\section{Discussion and Conclusion}
The appliance-aware bulk-plus-local model outperforms geometry-free interpolation under controlled truth, while residual calibration improves the sparse real-room point under date-block resampling. Public data support aligned subtasks, not arbitrary-building generalization or causal control. Future work will add longer deployment, dense measured truth, interventions, humidity alignment, and a preregistered same-row unfiltered/moving-average/KF/EKF comparison after defining the state, observation, and covariance models; Kalman performance is not yet evaluated.
