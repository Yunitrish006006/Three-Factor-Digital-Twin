...
\subsection{BMC Data-Correction Chain}

E12 stops before final-test access because six development files have fewer than 30 valid rows; E13 remains parser-invalidated. E14A/B preserve 4,038 source-correct rows and normalize three raw-unit files. On the already-opened E14C retrospective set, frozen load-aware ridge reduces MAE/RMSE/P95 from 4.0882/5.2087/12.0000 to 1.8054/2.8001/7.1146~$^\circ$C, wins 13/14 runs, and yields a run-block MAE-gain interval of $[1.4271,2.7939]~^\circ$C. This supports candidate eligibility only. The separately preregistered E15 confirmation evaluates 14 previously unopened files (3,112 rows) once without refitting: MAE/RMSE/P95 decrease from 1.6939/1.9672/3.5000 to 0.9744/1.2056/2.3888~$^\circ$C, with 12/14 run wins. Macro run MAE gain is 0.6134~$^\circ$C with a 20,000-replicate run-block bootstrap 95\% interval of $[0.2721,0.9831]~^\circ$C; all ten preregistered gates pass, supporting H-ENC-08. Runs \texttt{202308051757.csv} and \texttt{202310252230.csv} retain adverse MAE gains of $-0.5537$ and $-0.3011~^\circ$C. Support is limited to same-server temporal/workload transfer; PC enclosures, NTC hardware, cross-server transfer, spatial fields, and control efficacy remain unverified. Enclosures are the active application direction within the broader single-room thesis scope.
