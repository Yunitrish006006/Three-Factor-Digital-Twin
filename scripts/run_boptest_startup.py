...
import boptest_startup_adapter as adapter
from run_boptest_cross_plant import ROOT, sha, fmu_path
from select_boptest_startup import ART, windows
...
def source_hashes():
    ...
    paths += [ROOT/'scripts'/n for n in ('boptest_startup_adapter.py','select_boptest_startup.py','run_boptest_cross_plant.py')]
    paths += [ROOT/'digital_twin/control'/n for n in ('startup_pi.py','portable_pi.py','delayed_dynamics.py')]
...
def main():
    ...
    if phase=='confirmation':
        ...
        bands=[None,selection['selections'][name]['band_C']];days=[35,70];hours=24
    else:bands=[None,.05,.1,.2];days=[8];hours=3
    ...
    try:
        for day in days:
            ...
            for j,band in enumerate(bands):
                ...
                ctrl=StartupPI(c['target_C'],c['step_s'],model,band)
                rows,rec=adapter.episode(c,f'{phase}_{day}_{band}',day*86400,hours,ctrl)
                rec.update(day=day,band_C=band,method='auto_pi' if band is None else 'startup',reused=False,
                           windows=windows(rows,c),exit_code=ctrl.exit_code,exit_min=ctrl.exit_min)
