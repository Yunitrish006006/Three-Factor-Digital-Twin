...
def episode(c, name, start, hours, controller):
    ...
    path = ROOT / 'outputs/boptest_predictive_startup' / c['case'] / (name + '.csv')
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='boptest-native-') as temp:
            ...
            try:
                ...
                for o in c['overrides']:
                    fmu.setBoolean([vr[o['activate']]], [True]); fmu.setReal([vr[o['input']]], [294.15 if o['input'] in c['startup_target_inputs'] else o['value']])
                ...
                for o in c['overrides']:
                    if o['input'] in c['startup_target_inputs']:
                        fmu.setReal([vr[o['input']]], [o['value']])
