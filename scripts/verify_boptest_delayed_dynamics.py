...
import csv,json
from run_boptest_delayed_dynamics import ART,ROOT,CONFIG,source_hashes,sha
from run_boptest_cross_plant import metrics,encode
...
def main():
    ...
    for name in ['air','hydronic']:
        ...
        c=r['config'];cal=loadrows(ROOT/r['calibration_trace'])
        ...
        initial={}
        for rec in r['trials']+r['evaluations']:
            ...
            for a,b in zip(rows,rows[1:]):
                ...
                assert abs(encode(c,a['u'])-b['supply_C'])<1e-9
            for row in rows:assert 0<=row['u']<=1 and abs(row['aux']-c['aux_value'])<1e-9
    ...
    output['new_traces']=total;output['scope']='DEVELOPMENT_PLANTS_WITH_NEW_DATE_CHECKS'
