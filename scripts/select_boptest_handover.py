...
ART=ROOT/'openspec/changes/startup-handover-ablation/artifacts'
...
def select(records):
    base=records[0]['windows']; decisions=[dict(policy=r['policy'],**gate(base,r['windows'])) for r in records[1:]]
    eligible=[r for r,d in zip(records[1:],decisions) if d['passed']]
    chosen=min(eligible,key=lambda r:(r['windows']['early']['mae_C'],int(r['policy'].split('_')[0]),r['policy'].endswith('retain'))) if eligible else records[0]
    return dict(policy=chosen['policy'],decisions=decisions)
