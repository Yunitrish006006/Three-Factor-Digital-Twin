...
import boptest_handover_adapter as adapter
from run_boptest_cross_plant import ROOT, sha, fmu_path
from select_boptest_handover import ART, windows
...
def source_hashes():
    ...
    paths += [ROOT/'scripts'/n for n in ('boptest_handover_adapter.py','select_boptest_handover.py','run_boptest_cross_plant.py')]
    paths += [ROOT/'digital_twin/control'/n for n in ('handover_pi.py','startup_pi.py','portable_pi.py','delayed_dynamics.py')]
...
def main():
    ...
    if phase=='confirmation':
        ...
        bands=[None,selection['selections'][name]['policy']];days=[48,83];hours=24
    else:bands=[None,'15_taper','15_retain','55_taper','55_retain'];days=[8];hours=3
        ...
        for day in days:
            ...
            for j,band in enumerate(bands):
                if phase=='development' and j==0:
                    prior=ROOT/'openspec/changes/startup-pi-boundary/artifacts'/(name+'_development.json')
                    rec=dict(json.loads(prior.read_text())['evaluations'][0],policy=None,reused=True,reference_source=str(prior.relative_to(ROOT)))
                ...
                ctrl=HandoverPI(c['target_C'],c['step_s'],model,band)
                rows,rec=adapter.episode(c,f'{phase}_{day}_{band}',day*86400,hours,ctrl)
                rec.update(day=day,policy=band,method='auto_pi' if band is None else 'startup',reused=False,
