...
ART=ROOT/'openspec/changes/startup-pi-boundary/artifacts'
...
def select(records):
    base=records[0]['windows']; decisions=[dict(band_C=r['band_C'],**gate(base,r['windows'])) for r in records[1:]]
    eligible=[r for r,d in zip(records[1:],decisions) if d['passed']]
    chosen=min(eligible,key=lambda r:(r['windows']['early']['mae_C'],-r['band_C'])) if eligible else records[0]
    return dict(band_C=chosen['band_C'],decisions=decisions)
