...
def ensure_image_asset(block: Block) -> Path:
    ...
    asset_dir_value = block.get("output_dir")
    asset_dir = Path(str(asset_dir_value)) if asset_dir_value else PAPER_ASSETS
    asset_dir.mkdir(parents=True, exist_ok=True)
    asset_name = str(block.get("asset_name", source.stem))
    if source.suffix.lower() == ".svg":
        target = asset_dir / f"{asset_name}.png"
        if target.exists() and target.stat().st_mtime >= source.stat().st_mtime:
            return target
        qlmanage = shutil.which("qlmanage")
        imagemagick = shutil.which("convert")
        if qlmanage is None and imagemagick is None:
            raise SystemExit("qlmanage or ImageMagick convert is required to render SVG figures.")
        with tempfile.TemporaryDirectory(prefix="thesis_svg_", dir=tempfile.gettempdir()) as temp_name:
        ...
        return target
    target = asset_dir / f"{asset_name}{source.suffix.lower()}"
...
def build_blocks() -> List[Block]:
    return [
        ...
        paragraph("  5.7 【實驗 E6】Pure RNN 與 Hybrid Residual Neural Network 結果…… 15"),
        ...
        heading("5.7 【實驗 E6】Pure RNN 與 Hybrid Residual Neural Network 結果", 2),
        paragraph(
        ...
        heading("5.9.3.3 Kalman Filter 受控同資料比較", 4),
        paragraph(
        ...
        heading("5.9.3.4 機箱 BMC 公開資料轉移比較", 4),
        paragraph(
        ...
        heading("5.9.3.5 AAU 伺服器機房空間轉移比較", 4),
        paragraph(
        ...
        heading("5.9.3.6 E11C 局部鄰域獨立確認", 4),
        paragraph(
        ...
        heading("5.9.3.7 BMC 跨 run 資料修正、回溯敏感度與同伺服器確認", 4),
        paragraph(
        ...
        paragraph(
            "E14C 在 E14B 修正資料上重跑未改動的 candidate family。Load-aware ridge 的 macro MAE 改善之 run-block bootstrap 95% 區間為 [1.4271, 2.7939] °C，且預測落在 37.0033–64.3640 °C，因此通過候選資格閘門。然而這 14 個 test files 已在 E13 與 parser 除錯期間開啟，E14C 只能稱 retrospective sensitivity，不能稱 unseen confirmation。"
        ),
        paragraph(
            "E15 在檔案、模型內容 hash 與十項閘門預先固定後，對另 14 個先前未開啟檔案完成一次不重訓確認，共保留 3,112 rows。"
        ),
        table(
            ["E15 同伺服器 temporal/workload confirmation", "MAE（°C）", "RMSE（°C）", "P95（°C）", "逐 run 勝出"],
            [
                ["Inlet + frozen offset baseline", "1.6939", "1.9672", "3.5000", "2/14"],
                ["Frozen load-aware ridge", "0.9744", "1.2056", "2.3888", "12/14"],
            ],
        ),
        paragraph(
            "E15 的 macro run MAE 改善為 0.6134 °C，20,000 次 run-block bootstrap 95% 區間為 [0.2721, 0.9831] °C；十項閘門全數通過，因此 H-ENC-08 判定支持。仍保留 202308051757.csv 與 202310252230.csv 的負向結果，MAE 改善分別為 -0.5537 與 -0.3011 °C。此確認僅支持同一伺服器的 temporal/workload transfer，不驗證個人電腦機箱、NTC 硬體、跨伺服器轉移、3-D 空間熱場或控制效益；機箱是目前應用推進方向，單房間研究仍保留為論文整體範圍。"
        ),
        ...
        bullets(
            [
                ...
                "BMC E12 因 6 個 development files 未達 30-row 門檻而未評估 final test；E13 為 parser-invalidated。E14C 仍只構成回溯候選資格證據；E15 雖通過十項閘門、12/14 runs 勝出，但只支持同伺服器 temporal/workload transfer，且兩個退步 runs 必須保留。",
...
# Synchronized E11D appendix; the main builder recreates outputs before this post-process.
if __name__ == "__main__":
    from pathlib import Path as _E11DPath
    import shutil as _e11d_shutil
    import tempfile as _e11d_tempfile
    import zipfile as _e11d_zipfile
    from xml.etree import ElementTree as _e11d_etree
...
"""
    _markdown_path = _e11d_root / "docs/thesis/thesis_draft_zh.md"
    if "H-ENC-04" not in _markdown_path.read_text(encoding="utf-8"):
        with _markdown_path.open("a", encoding="utf-8") as _handle:
            _handle.write(_e11d_markdown)
    _e11e_markdown = """
...
"""
    if "no_candidate_forwarded" not in _markdown_path.read_text(encoding="utf-8"):
        with _markdown_path.open("a", encoding="utf-8") as _handle:
            _handle.write(_e11e_markdown)
...
"""
    _current_markdown = _markdown_path.read_text(encoding="utf-8")
    if "RNN 同資料比較為 `COMPLETE`" not in _current_markdown or "Kalman 受控同資料比較為 `COMPLETE`" not in _current_markdown:
        with _markdown_path.open("a", encoding="utf-8") as _handle:
            _handle.write(_verification_status)

    _source_docx = _e11d_root / "docs/papers/thesis/thesis_draft_zh.docx"
    _output_docx = _e11d_root / "outputs/papers/thesis_draft_zh.docx"
    _output_docx.parent.mkdir(parents=True, exist_ok=True)
    _e11d_shutil.copy2(_source_docx, _output_docx)
    for _relative in (
        "docs/papers/thesis/thesis_draft_zh.docx",
        "outputs/papers/thesis_draft_zh.docx",
    ):
        _path = _e11d_root / _relative
        with _e11d_zipfile.ZipFile(_path, "r") as _archive:
            _entries = {name: _archive.read(name) for name in _archive.namelist()}
        _w = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
        _e11d_etree.register_namespace("w", _w)
        _root_xml = _e11d_etree.fromstring(_entries["word/document.xml"])
        _body = _root_xml.find(f"{{{_w}}}body")
        _section = _body.find(f"{{{_w}}}sectPr")
        _insert_at = list(_body).index(_section) if _section is not None else len(_body)

        def _append_paragraph(_text: str, _style: str | None = None) -> None:
            nonlocal_insert = _append_paragraph.__dict__.setdefault("index", _insert_at)
            _paragraph = _e11d_etree.Element(f"{{{_w}}}p")
            if _style:
                _properties = _e11d_etree.SubElement(_paragraph, f"{{{_w}}}pPr")
                _style_node = _e11d_etree.SubElement(_properties, f"{{{_w}}}pStyle")
                _style_node.set(f"{{{_w}}}val", _style)
            _run = _e11d_etree.SubElement(_paragraph, f"{{{_w}}}r")
            _text_node = _e11d_etree.SubElement(_run, f"{{{_w}}}t")
            _text_node.text = _text
            _body.insert(nonlocal_insert, _paragraph)
            _append_paragraph.__dict__["index"] = nonlocal_insert + 1

        _append_paragraph("E11D 機箱角色條件式獨立確認", "Heading2")
        _append_paragraph(
            "H-ENC-04 在 11 個獨立 4 MiB AAU v4 ranges、1,505 個一分鐘快照上判定為 supported。"
            "全域平均的 MAE/RMSE/P95 為 2.3972/2.9748/5.7232 C；固定角色條件模型為 "
            "1.6517/2.3648/5.4886 C，逐感測器勝出 30/42。"
        )
        _append_paragraph(
            "配對 MAE 改善為 0.7455 C；13 個日區塊、20,000 次 bootstrap 的 95% CI 為 "
            "[0.6867, 0.8124] C。此結果只支持角色語意的預測資訊，不證明氣流因果。"
        )
        _append_paragraph("E11E 分層角色局部模型開發結果", "Heading2")
        _append_paragraph(
            "E11E 的較強 local IDW baseline 為 MAE/RMSE/P95 1.1168/1.7250/3.4900 C；"
            "最佳 role_local_k5_p2 為 1.0187/1.6792/3.7699 C。"
        )
        _append_paragraph(
            "候選的 P95 惡化且只贏 25/42，未達 26/42，因此為 no_candidate_forwarded；"
            "E11F 保持未下載，不能宣稱確認改善。"
        )
        _entries["word/document.xml"] = _e11d_etree.tostring(
            _root_xml, encoding="utf-8", xml_declaration=True
        )
        with _e11d_tempfile.NamedTemporaryFile(dir=_path.parent, delete=False) as _temporary:
            _temporary_path = _E11DPath(_temporary.name)
        with _e11d_zipfile.ZipFile(_temporary_path, "w", _e11d_zipfile.ZIP_DEFLATED) as _archive:
            for _name, _payload in _entries.items():
                _archive.writestr(_name, _payload)
        _temporary_path.replace(_path)

# Keep generated thesis archives synchronized with the frozen E11G evidence.
if __name__ == "__main__":
    from sync_e11g_generated_artifacts import sync_docx_outputs

    sync_docx_outputs()


def e11g_blocks():
    """Return the frozen E11G section shared by non-DOCX renderers."""
    return [
        {
            "type": "heading",
            "text": "E11G tail-safe 自適應開發與空間覆蓋限制",
            "level": 1,
        },
        {
            "type": "paragraph",
            "text": (
                "E11G 針對 E11E 的 P95 惡化問題，在同一開發資料上預註冊 12 日 "
                "leave-one-day-out。方法以 local-IDW（k=3、p=2）為安全基線，以同角色 "
                "local-IDW（k=5、p=2）為專家，評估 30 組裁切修正與高分歧回退規格。"
                "每一折的感測器選擇只能使用其餘日期，候選必須同時改善 MAE、RMSE、P95 "
                "至少 0.02°C，並在至少 60% 訓練日降低 MAE；否則測試日回退基線。"
            ),
        },
        {
            "type": "paragraph",
            "text": (
                "在 42 個感測器、12 日與 63,084 筆感測器分鐘評估上，out-of-fold MAE "
                "由 1.1168°C 降至 0.8945°C，RMSE 由 1.7250°C 降至 1.5415°C，P95 由 "
                "3.4900°C 降至 3.1013°C；日區塊 MAE 改善 bootstrap 95% CI 為 "
                "[0.1847, 0.2620]°C。然而嚴格感測器勝率僅 21/42，另有 20 個因安全回退"
                "而持平、1 個微幅惡化，未達預註冊 26/42。因此決策為 "
                "no_candidate_forwarded，E11F 未存取；本結果僅屬適應性開發證據。"
            ),
        },
    ]
...
def e11hf_blocks():
    """Return frozen E11H development and E11F confirmation blocks."""
    return [
        {"type": "heading", "text": "E11H commissioning 開發與 E11F 凍結確認", "level": 1},
        {"type": "paragraph", "text": "E11H 以最早兩日目標點真值做 robust residual calibration，第三日選模，後續九日凍結測試。相較 local-IDW，MAE/RMSE/P95 由 1.0958/1.7435/3.5061°C 降至 0.4039/0.6830/1.2900°C；39/42 感測器改善，日區塊 95% CI 為 [0.4854, 0.9271]°C。多個 Huber slope 位於 0.5 邊界，affine 參數不可解讀為物理係數。"},
        {"type": "paragraph", "text": "E11F 使用預留 unseen byte ranges 與完全凍結的 42 組模型，不重新校正。MAE/RMSE/P95 由 1.1399/1.7850/3.5735°C 降至 0.3966/0.6723/1.2756°C；39/42 感測器改善，13 日 bootstrap 95% CI 為 [0.5851, 0.9274]°C。因此 h_enc_05_supported_within_campaign。11 個日期與 E11G 重疊、8 個與 E11H 重疊，故不是跨日期、跨機箱、氣流因果或 NTC 硬體驗證。"},
    ]


# Keep DOCX and Markdown synchronized with E11H/E11F evidence.
if __name__ == "__main__":
