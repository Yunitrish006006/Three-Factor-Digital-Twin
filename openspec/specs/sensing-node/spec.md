...
### Requirement: V1 input-grade hardware uses ESP32-C3, DHT11, and digital light sensor

V1 input-grade 節點 MAY 以 ESP32-C3 作為無線 MCU，DHT11 作為低成本溫濕度感測器，並優先使用 BH1750 或等價 I2C digital lux sensor 作為照度感測器。
...
### Requirement: Validation-grade nodes use higher-accuracy temperature humidity sensors

用於 `S_validation` 的 target-point validation nodes SHOULD 使用高於 DHT11 等級的溫濕度感測器，例如 SHT31、SHT35、SHT40、SHT45 或同等級元件；DHT11 可作為低成本 `S_input`，但不應被描述為高精度 validation truth。
